...
CREATE
OR REPLACE TRIGGER TRDEPT_COPIA_EXACTA
AFTER
INSERT
    OR DELETE
    OR
UPDATE
    ON DEPT FOR EACH ROW BEGIN IF UPDATING THEN
UPDATE
    DEPT_COPIA_EXACTA
SET
    DNAME = :NEW.DNAME,
...
IF INSERTING THEN
INSERT INTO
    DEPT_COPIA_EXACTA(DEPTNO, DNAME, LOC)
VALUES
...
WHERE
     ...
     */
    CREATE
    OR REPLACE PROCEDURE spDatosCli (nomcli CLIENTES.NOMBRECLIENTE % TYPE) IS V_PAGADO INTEGER := 0;
...
SELECT
    SUM(DP.CANTIDAD * DP.PRECIOUNIDAD) INTO V_GASTADO
FROM
    CLIENTES C,
    PEDIDOS P,
    DETALLEPEDIDOS DP
...
dbms_output.put_line(
    upper(
        'EL CLIENTE ' || nomcli || ' es atendido por ' || v_nom_emp || ' se ha gastado ' || v_gastado || ' y ha pagado ' || v_pagado
    )
);
...
loop fetch c_cli into v_nomcli;
